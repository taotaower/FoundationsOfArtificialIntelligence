...
class ExactInference(InferenceModule):
    ...
    #    util.raiseNotDefined()
        ...
        for p in self.legalPositions:
            manhattanDis = util.manhattanDistance(p, pacmanPosition)
            allPossible[p] = self.beliefs[p] * emissionModel[manhattanDis]
...
#        util.raiseNotDefined()
        ...
        for p in pos:
            ...
            for posProPair in newPosDist.items():
                position = posProPair[0]
                prob = posProPair[1]
                newBeliefs[position] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for num in range(self.numParticles):
            partPos = pos [num % len(pos)]
            self.particles.append(partPos)
  ...
  #      util.raiseNotDefined()
        ...
        if noisyDistance >= 0:
            ...
            for p in pos:
                ...
                prob[p] += emissionModel[dis] * belief[p]
            if prob.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for num in range(nums):
                    self.particles[num] = util.sample(prob)
        else:
            self.particles = jailPos * nums
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles :
            gPos =(self.setGhostPosition(gameState, pos))
            sample = util.sample(self.getPositionDistribution(gPos))
            newPar.append(sample)
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for num in range(self.numGhosts):
                particleInJail = self.getParticleWithGhostInJail(particle, num)
                dis = util.manhattanDistance(particle[num], pacmanPosition)
                if noisyDistances[num] >= 0 :
                    div *= emissionModels[num][dis]
                else:
                    particle = particleInJail

            allPossible[particle] += div
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            gosPos =setGhostPositions(gameState, newParticle)
            newParticle = [util.sample (getPositionDistributionForGhost(gosPos, num, self.ghostAgents[num]))
                           for num in range(self.numGhosts)]
            "*** END YOUR CODE HERE ***"
