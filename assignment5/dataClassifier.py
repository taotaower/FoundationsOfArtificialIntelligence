...
#    util.raiseNotDefined()
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            features[(x, y)] = 0 if datum.getPixel(x, y) <= 0 else 1
    num += 1
    ...
    for x in range(DIGIT_DATUM_HEIGHT):
        ...
        for y in range(DIGIT_DATUM_WIDTH):
            ...
            if feature == 0 and k > 0:
                feature = 1
            if pix == 0 and feature == 1 and k == 0 :
                pix = 1
            if feature == 1 and pix == 1 and k > 0 :
                otherFeat = 1
            if pix == 1 and otherFeat == 1 and feature == 1 and k == 0:
                freqPix = freqPix + 1
                feature = pix = otherFeat = 0
            features[num] = 1 if freqPix > 0 else 0
            num += 1
    return features
...
#    util.raiseNotDefined()
    ...
    features['food'] = nextState.getFood().count()
    features['gDistance'] = \
        min(util.manhattanDistance(loc, nextState.getPacmanPosition())
                          for loc in nextState.getGhostPositions())
    return features
